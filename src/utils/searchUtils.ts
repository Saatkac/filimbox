...
// Levenshtein mesafesi hesapla (fuzzy matching için)
...
// Fuzzy match - yazım hatalarına toleranslı eşleşme
export const fuzzyMatch = (text: string, query: string, maxDistance: number = 2): boolean => {
  if (!text || !query) return false;
  ...
  const textWords = normalizedText.split(/[\s\-_:.,'!?()]+/).filter(w => w.length >= 2);
  
  for (const word of textWords) {
    // Dynamic max distance based on word length
    const effectiveMaxDistance = normalizedQuery.length <= 3 ? 1 : 
                                  normalizedQuery.length <= 5 ? 2 : 
                                  Math.min(3, maxDistance);
    
    // Check if query is similar to any word in the text
    const distance = levenshteinDistance(word, normalizedQuery);
    if (distance <= effectiveMaxDistance) {
      return true;
    }
    
    // Also check if query is a prefix with small distance
    if (word.length >= normalizedQuery.length) {
      const prefix = word.substring(0, normalizedQuery.length);
      const prefixDistance = levenshteinDistance(prefix, normalizedQuery);
      if (prefixDistance <= 1) {
        return true;
      }
    }
